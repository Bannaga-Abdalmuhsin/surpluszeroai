...
def main():
 p=argparse.ArgumentParser();p.add_argument("--config",default="config/riyadh_demo.json");p.add_argument("--output",default="output/riyadh_demo");a=p.parse_args();result=run(a.config,a.output);print(json.dumps({"surplus":result["summary"],"dispatch":result["dispatch_summary"]},indent=2))
